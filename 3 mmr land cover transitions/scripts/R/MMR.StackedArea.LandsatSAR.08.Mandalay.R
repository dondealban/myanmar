...
setwd("/Users/dondealban/Dropbox/Research/myanmar/3 mmr land cover transitions/stacked area/8_mandalay")
...
temp <- rbind(temp1, temp2a, temp3a)
...
plot <- ggplot() + geom_area(aes(x=Years, y=Percentage, fill=factor(Class,
                   labels=c("Built-up",
                            ...
                            "Bare Ground"))), 
                   data=data)
plot <- plot + labs(title="Net Land Cover Transitions: Mandalay Region", x="Year", y="Percentage of Landscape", fill="Land Cover Category")
...
ggsave(plot, file="StackedArea-08-Mandalay.pdf", width=19.89, height=15, units="cm", dpi=300)
