...
setwd("/Users/dondealban/Dropbox/Research/myanmar/3 mmr land cover transitions/stacked area/7_magway")
...
plot <- ggplot() + geom_area(aes(x=Years, y=Percentage, fill=factor(Class,
                   labels=c("Built-up",
                            ...
                            "Bare Ground"))), 
                   data=data)
plot <- plot + labs(title="Net Land Cover Transitions: Magway Region", x="Year", y="Percentage of Landscape", fill="Land Cover Category")
...
ggsave(plot, file="StackedArea-07-Magway.pdf", width=19.89, height=15, units="cm", dpi=300)
