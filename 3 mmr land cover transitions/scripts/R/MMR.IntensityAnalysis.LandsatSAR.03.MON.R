...
# ----------------------------------------
setwd("/Users/dondealban/Dropbox/Research/myanmar/3 mmr land cover transitions/mangroves/intensity analysis/3_mon")
...
# LOAD RASTER DATA
# ----------------------------------------
r1996 <- raster('MMR_1996_MON_RCL.tif')
r2007 <- raster('MMR_2007_MON_RCL.tif')
r2016 <- raster('MMR_2016_MON_RCL.tif')
...
IIAname <- file.path(normalizePath(getwd()), "IntensityAnalysis_MON_IIA.csv")
IIA2csv(IIA.output, time.points, IIAname)
CIAname <- file.path(normalizePath(getwd()), "IntensityAnalysis_MON_CIA.csv")
CIA2csv(CIA.output, time.points, categories, CIAname)
TIAname <- file.path(normalizePath(getwd()), "IntensityAnalysis_MON_TIA.csv")
...
write.csv(crosstab_1, "IntensityAnalysis_MON_CrossTab_I1.csv")
crosstab_2 <- crosstabulation[[2]] #2007-2016
write.csv(crosstab_2, "IntensityAnalysis_MON_CrossTab_I2.csv")
