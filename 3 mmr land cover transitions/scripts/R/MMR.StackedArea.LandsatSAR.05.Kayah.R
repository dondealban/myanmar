...
setwd("/Users/dondealban/Dropbox/Research/myanmar/3 mmr land cover transitions/stacked area/5_kayah")
...
temp1a <- t(as.matrix(c(temp1[,1:2], 0, 0, temp1[,3:ncol(temp1)]))) # transposed 1x10 matrix
temp2a <- t(as.matrix(c(temp2[,1:2], 0, 0, 0, temp2[,3], 0, temp2[,4:ncol(temp2)]))) # transposed 1x10 matrix
temp3a <- t(as.matrix(c(temp3[,1:2], 0, temp3[,3], 0, temp3[,4:ncol(temp3)]))) # transposed 1x10 matrix
...
plot <- ggplot() + geom_area(aes(x=Years, y=Percentage, fill=factor(Class,
                   labels=c("Built-up",
                            ...
                            "Bare Ground"))), 
                   data=data)
plot <- plot + labs(title="Net Land Cover Transitions: Kayah State", x="Year", y="Percentage of Landscape", fill="Land Cover Category")
...
ggsave(plot, file="StackedArea-05-Kayah.pdf", width=19.89, height=15, units="cm", dpi=300)
