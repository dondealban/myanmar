...
setwd("/Users/dondealban/Dropbox/Research/myanmar/3 mmr land cover transitions/stacked area/10_naypyitaw")
...
# Create new column with zeroes for missing classes in each period and insert in matrix
temp1a <- t(as.matrix(c(temp1[,1:2], 0, 0, 0, temp1[,3], 0, temp1[,4:ncol(temp1)]))) # transposed 1x10 matrix
temp2a <- t(as.matrix(c(temp2[,1:2], 0, temp2[,3], 0, temp2[,4], 0, temp2[,5:ncol(temp2)]))) # transposed 1x10 matrix
...
plot <- ggplot() + geom_area(aes(x=Years, y=Percentage, fill=factor(Class,
                   labels=c("Built-up",
                            ...
                            "Bare Ground"))), 
                   data=data)
plot <- plot + labs(title="Net Land Cover Transitions: Naypyitaw Region", x="Year", y="Percentage of Landscape", fill="Land Cover Category")
...
ggsave(plot, file="StackedArea-10-Naypyitaw.pdf", width=19.89, height=15, units="cm", dpi=300)
