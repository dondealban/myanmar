...
setwd("/Users/dondealban/Dropbox/Research/myanmar/3 mmr land cover transitions/mangroves/stacked area/4_rakhine")
...
plot <- ggplot() + geom_area(aes(x=Years, y=Percentage, fill=factor(Class,
                   labels=c("Built-up",
                            ...
                            "Bare Ground"))), 
                   data=data)
plot <- plot + labs(title="Net Land Cover Transitions: Rakhine State", x="Year", y="Percentage of Landscape", fill="Land Cover Category")
...
ggsave(plot, file="StackedArea-11-Rakhine.pdf", width=16, height=15, units="cm", dpi=300)
