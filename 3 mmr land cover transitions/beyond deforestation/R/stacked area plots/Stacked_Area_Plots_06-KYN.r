...
setwd("C:/MMR-LCC-EndAugust/03-QGIS-SCP-CR/06-KYN")
...
plot <- ggplot() + geom_area(aes(x=Years, y=Percentage, fill=factor(Class,
                   labels=c("Bare Ground",
                            ...
                            "Water Body"))),
                   data=data)
plot <- plot + labs(title="Kayin Region", x="Year", y="Percentage of Landscape", fill="Land Cover Category")
...
ggsave(plot, file="C:/MMR-LCC-EndAugust/04-STACKED-AREA-PLOTS/Stacked-Area-06-KYN.pdf", width=16, height=15, units="cm", dpi=300)
