...
setwd("C:/MMR-LCC-EndAugust/03-QGIS-SCP-CR/03-CHI")
...
plot <- ggplot() + geom_area(aes(x=Years, y=Percentage, fill=factor(Class,
                   labels=c("Bare Ground",
                            ...
                            "Water Body"))),
                   data=data)
plot <- plot + labs(title="Chin Region", x="Year", y="Percentage of Landscape", fill="Land Cover Category")
...
ggsave(plot, file="C:/MMR-LCC-EndAugust/04-STACKED-AREA-PLOTS/Stacked-Area-03-CHI.pdf", width=16, height=15, units="cm", dpi=300)
