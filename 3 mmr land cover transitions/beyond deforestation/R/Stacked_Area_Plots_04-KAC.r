...
setwd("C:/MMR-LCC-EndAugust/03-QGIS-SCP-CR/04-KAC-REVISED")

# The revised version is used to ensure all files have 9 LC classes for all timeponts by filling in 0 values.
...
plot <- ggplot() + geom_area(aes(x=Years, y=Percentage, fill=factor(Class,
                   labels=c("Bare Ground",
                            ...
                            "Water Body"))),
                   data=data)
plot <- plot + labs(title="Kachin Region", x="Year", y="Percentage of Landscape", fill="Land Cover Category")
...
ggsave(plot, file="C:/MMR-LCC-EndAugust/04-STACKED-AREA-PLOTS/Stacked-Area-04-KAC.pdf", width=16, height=15, units="cm", dpi=300)
